...
void Md5::Calculate(const char *content, unsigned int len, char *md5) {
	MD5Context context;
	unsigned char buf[16];
	
	MD5_Init(&context);
	MD5_Update(&context, (unsigned char *)content, len);
	MD5_Final(&context, buf);
	int i;
	for (i = 0; i < 16; i ++) {
		sprintf(md5 ++, "%x", (buf[i] & 0xF0) >> 4);
		sprintf(md5 ++, "%x", buf[i] & 0x0F);
	}
}
