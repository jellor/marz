...
class SessionSvr : public Svr {
public:
	SessionSvr() {
		DLOG << "SessionSvr Svr Constructor ......";
	}
	~SessionSvr() {
		DLOG << "SessionSvr Svr Destructor ......";
	}
...
int main(int argc, char* argv[]) {
	...
	Config config("session_svr.conf");
	...
	if (client_listen_hosts == NULL || client_listen_port == NULL) {
		WLOG << "Error : session_svr.conf Failed";
	...
	SessionSvr* server = new SessionSvr();
