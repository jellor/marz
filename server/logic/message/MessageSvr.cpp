...
class MessageSvr : public Svr {
public:
	MessageSvr() {
		DLOG << "MessageSvr Svr Constructor ......";
	}
	~MessageSvr() {
		DLOG << "MessageSvr Svr Destructor ......";
	}
...
int main(int argc, char* argv[]) {
	...
	Config config("message_svr.conf");
	...
	if (client_listen_hosts == NULL || client_listen_port == NULL) {
		WLOG << "Error : message_svr.conf Failed";
	...
	MessageSvr* server = new MessageSvr();
