...
class RelationSvr : public Svr {
public:
	RelationSvr() {
		DLOG << "RelationSvr Svr Constructor ......";
	}
	~RelationSvr() {
		DLOG << "RelationSvr Svr Destructor ......";
	}
...
int main(int argc, char* argv[]) {
	...
	Config config("relation_svr.conf");
	...
	if (client_listen_hosts == NULL || client_listen_port == NULL) {
		WLOG << "Error : relation_svr.conf Failed";
	...
	RelationSvr* server = new RelationSvr();
